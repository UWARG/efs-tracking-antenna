...
class trackingAntenna
{
  private:
    ...
    void getTriangleSidesLength(double lat_2)
    {
      ...
      std::cout << "Lat Seg, Long Seg, Vector: " << lat_seg << " " << long_seg << " " << vector << "\n";
    }
  ...
  public:
    ...
    std::string getYawAndPitch(double lat_1, double long_1, float alt_1, double lat_2, double long_2, float alt_2) //returns "yaw,pitch"
    {
      ...
      getTriangleSidesLength(lat_2); //calculates the lengths of straight segments that are located in a plane
      ...
      pitch_rot = radianToDeg(atan((cos(alpha)*alt_diff - CURVE_ADJ_FACTOR*vector)/vector)) * SCALE_DOWN; 
